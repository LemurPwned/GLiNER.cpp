...
PYBIND11_MODULE(gliner_cpp, m) {
    m.doc() = "Pybind11 bindings for the GLiNER model";
    ...
    py::class_<gliner::Config>(m, "Config")
        .def(py::init([](int max_width, int max_length, gliner::ModelType model_type) {
            ...
            "Create a configuration for the GLiNER model.")
    ...
    py::class_<gliner::Model>(m, "Model")
        ...
        .def(py::init<const std::string &, const std::string &, const gliner::Config &, int>(),
             ...
             "Create a GLiNER model with an explicit device id (>=0 uses CUDA, <0 runs on CPU).")
        .def("inference",
             [](gliner::Model &self,
             ...
             py::arg("texts"), py::arg("entities"),
             py::arg("flat_ner") = true, py::arg("threshold") = 0.5f, py::arg("multi_label") = false,
             "Run inference for a batch of texts and entities.");
}
